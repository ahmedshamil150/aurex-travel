...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && id) {
      const { data, error } = await supabase.from('drivers').select('*, vehicles(*)').eq('id', id).single();
    ...
    if (req.method === 'GET') {
      ...
      if (search) {
        query = query.or(`first_name.ilike.%${search}%,last_name.ilike.%${search}%,mobile.ilike.%${search}%`);
      }
    ...
    if (req.method === 'PATCH') {
      ...
      const { data, error } = await supabase.from('drivers').update(updateData).eq('id', body.id).select().single();
      if (error) throw error;
      return res.status(200).json(data);
    }
